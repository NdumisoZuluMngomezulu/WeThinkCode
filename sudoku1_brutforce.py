...
def valid_play(board, row, column, choice):
    if 0 <= row < 9 and 0 <= column < 9 and choice not in board[row] and choice not in [rows[column] for rows in board]:
        first_grid = [rows[0:3] for rows in board[0:3]]
        first_grid = [element for row in first_grid for element in row]
        second_grid = [rows[3:6] for rows in board[0:3]]
        second_grid = [element for row in second_grid for element in row]
        third_grid = [rows[6:9] for rows in board[0:3]]
        third_grid = [element for row in third_grid for element in row]
        fourth_grid = [rows[0:3] for rows in board[3:6]]
        fourth_grid = [element for row in fourth_grid for element in row]
        fifth_grid = [rows[3:6] for rows in board[3:6]]
        fifth_grid = [element for row in fifth_grid for element in row]
        sixth_grid = [rows[6:9] for rows in board[3:6]]
        sixth_grid = [element for row in sixth_grid for element in row]
        seventh_grid = [rows[0:3] for rows in board[6:8]]
        seventh_grid = [element for row in seventh_grid for element in row]
        eight_grid = [rows[3:6] for rows in board[6:8]]
        eight_grid = [element for row in eight_grid for element in row]
        nineth_grid = [rows[6:9] for rows in board[6:8]]
        nineth_grid = [element for row in nineth_grid for element in row]
...
print(f"""
        ...
        I am iterating by rows first, then iterating by the columns in each row. This is a nested For Loop 
...
while zero_loop_condition(board):
    ...
    print(location)
    for i in range(9):
        if valid_play(board, location[0], location[1], i):
            board[location[0]][location[1]] = i
